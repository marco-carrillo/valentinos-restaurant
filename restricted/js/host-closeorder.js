...
function getDetails(){
    ...
    //*********************************************/
    //  checking that a valid selection was made
    //*********************************************/
    ...
    //******************************/
    let order=orderSelected[0].cells[1].textContent;  // Saves it so that the next window picks it up
    let table_name=orderSelected[0].cells[3].textContent;
    let table_id =table_name.charAt(table_name.length-1);
    console.log(table_id);
    console.log(table_name);

    $.post("/api/changeOrderStatus",{status_id: 5,id:order})
            .then(all_data=>{
 
                //*********************************************/
                //  changing  table occupied status to false
                //*********************************************/
                console.log(`about to change occupie ${table_id}`);
                $.post("/api/occupyTable",{id: table_id,occupied:false})
                    .then(all_data=>{
                        location.reload();
                    });
            });
